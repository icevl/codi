...
from codexbot.handlers.status_polling import update_status_message
...
class TestStatusPollerSettingsDetection:
    ...
    async def test_settings_ui_detected_and_keyboard_sent(
        ...
        with (
            patch("codexbot.handlers.status_polling.tmux_manager") as mock_tmux,
            patch(
                "codexbot.handlers.status_polling.handle_interactive_ui",
    ...
    async def test_normal_pane_no_interactive_ui(self, mock_bot: AsyncMock):
        ...
        with (
            ...
            patch(
                "codexbot.handlers.status_polling.enqueue_status_update",
                new_callable=AsyncMock,
            ),
        ):
            mock_tmux.find_window_by_id = AsyncMock(return_value=mock_window)
    ...
    async def test_settings_ui_end_to_end_sends_telegram_keyboard(
        ...
        with (
            patch("codexbot.handlers.status_polling.tmux_manager") as mock_tmux_poll,
            patch("codexbot.handlers.interactive_ui.tmux_manager") as mock_tmux_ui,
    ...
    @pytest.mark.asyncio
    async def test_exit_plan_ui_end_to_end_sends_telegram_keyboard(
        self, mock_bot: AsyncMock, sample_pane_exit_plan: str
    ):
        """Current plan-mode pane should be detected and forwarded by the poller."""
        window_id = "@5"
        mock_window = MagicMock()
        mock_window.window_id = window_id
        ...
        with (
            ...
            mock_tmux_poll.find_window_by_id = AsyncMock(return_value=mock_window)
            mock_tmux_poll.capture_pane = AsyncMock(return_value=sample_pane_exit_plan)
            mock_tmux_ui.find_window_by_id = AsyncMock(return_value=mock_window)
            mock_tmux_ui.capture_pane = AsyncMock(return_value=sample_pane_exit_plan)
            mock_sm.resolve_chat_id.return_value = 100
            ...
            assert "Implement this plan?" in call_kwargs["text"]

    @pytest.mark.asyncio
    async def test_exit_plan_current_ui_end_to_end_sends_telegram_keyboard(
        self, mock_bot: AsyncMock, sample_pane_exit_plan_current: str
    ):
        """Current Codex plan-mode wording should still be detected by the poller."""
        window_id = "@5"
        mock_window = MagicMock()
        mock_window.window_id = window_id
        ...
        with (
            ...
            mock_tmux_poll.capture_pane = AsyncMock(
                return_value=sample_pane_exit_plan_current
            )
            mock_tmux_ui.find_window_by_id = AsyncMock(return_value=mock_window)
            ...
            assert call_kwargs["message_thread_id"] == 42
            assert call_kwargs["reply_markup"] is not None
            assert "Implement the plan." in call_kwargs["text"]

    @pytest.mark.asyncio
    async def test_request_user_input_current_ui_end_to_end_sends_telegram_keyboard(
        self, mock_bot: AsyncMock, sample_pane_ask_user_current_multi: str
    ):
        """Current request_user_input bottom-pane overlay should be detected."""
        window_id = "@5"
        mock_window = MagicMock()
        mock_window.window_id = window_id
        ...
        with (
            ...
            mock_tmux_poll.find_window_by_id = AsyncMock(return_value=mock_window)
            mock_tmux_poll.capture_pane = AsyncMock(
                return_value=sample_pane_ask_user_current_multi
            )
            mock_tmux_ui.find_window_by_id = AsyncMock(return_value=mock_window)
            mock_tmux_ui.capture_pane = AsyncMock(
                return_value=sample_pane_ask_user_current_multi
            ...
            assert call_kwargs["message_thread_id"] == 42
            assert call_kwargs["reply_markup"] is not None
            assert "Question 1/2" in call_kwargs["text"]

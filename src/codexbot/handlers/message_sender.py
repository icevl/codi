...
Functions:
  ...
  - send_document: Document sending (single or media group)
...
def _named_bytes(raw_bytes: bytes, filename: str) -> io.BytesIO:
    """Build named in-memory file object for Telegram media methods."""
...
async def send_document(
    ...
    **kwargs: Any,
    ...
    """Send document(s) to chat. Sends as media group if multiple documents.
    ...
    Args:
        bot: Telegram Bot instance
        chat_id: Target chat ID
        document_data: List of (media_type, raw_bytes) tuples
        filename: Base filename used for Telegram media objects
    ...
    try:
        if len(document_data) == 1:
            _media_type, raw_bytes = document_data[0]
            await bot.send_document(
                chat_id=chat_id,
                document=_named_bytes(raw_bytes, filename),
                **kwargs,
            )
        else:
            media = [
                InputMediaDocument(media=_named_bytes(raw_bytes, filename))
                for _media_type, raw_bytes in document_data
            ]
            await bot.send_media_group(
                chat_id=chat_id,
                media=media,

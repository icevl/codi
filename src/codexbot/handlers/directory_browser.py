...
STATE_SELECTING_RUNTIME = "selecting_runtime"
PENDING_RUNTIME_KEY = "_pending_runtime"  # runtime name picked for the new session
...
def clear_runtime_picker_state(user_data: dict | None) -> None:
    """Clear runtime picker state keys from user_data."""
    if user_data is not None:
        user_data.pop(STATE_KEY, None)


def build_runtime_picker() -> tuple[str, InlineKeyboardMarkup]:
    """Build the agent runtime picker (Codex vs Claude Code)."""
    text = (
        "*Pick an agent*\n\n"
    ...
    buttons = [
        [
            InlineKeyboardButton("🔧 Codex", callback_data=CB_RUNTIME_CODEX),
            InlineKeyboardButton("🧠 Claude Code", callback_data=CB_RUNTIME_CLAUDE),
        ],
        [InlineKeyboardButton("Cancel", callback_data=CB_RUNTIME_CANCEL)],
    ]
